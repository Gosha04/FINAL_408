...
st.set_page_config(page_title = "Manage Sales", page_icon = "🧾", layout = "wide")
...
st.title(f"Welcome, {user["first_name"]} {user["last_name"]}! Manage Sales Here.")

view_sales = st.container(border = True)

with view_sales:
    st.subheader("View Filed Sales", divider = "gray")
    ...
    search_term = st.pills("Find by", options = ["All", "Customer ID", "Vehicle ID", "Amount"], default = "All")

    if search_term:
        if search_term == "All":
            st.dataframe(temp_sales_df, hide_index = True)

            st.download_button("Download CSV", file_name = f"emp{user["id"]}_all_sales.csv", data = temp_sales_df.to_csv().encode("utf-8"), icon = ":material/download:")
        else:
            ...
            if to_search_for:
                try:
                    ...
                    if search_term == "Amount":
                        asc_desc_eq_sales = st.segmented_control("Select criteria", ["Less than", "Equal to", 'Greater than'],
                                                           default="Equal to", key = "plsfix")

                        if asc_desc_eq_sales == "Less than":
                            temp_sales_df = temp_sales_df.loc[temp_sales_df[search_term] < to_search_for]

                            if temp_sales_df.empty:
                                st.warning("No results.")
                            else:
                                st.dataframe(temp_sales_df, hide_index=True)
                                st.download_button("Download CSV", data=temp_sales_df.to_csv().encode("utf-8"),
...
file_sale, delete_sale = st.columns(2, border = True)

with file_sale:
    st.subheader("File a Sale", divider = "gray")
    ...
    if new_cust:
        ...
        st.write("Enter Sale Date")
        year, month, day = st.columns(3)
        with year:
            file_year = st.text_input("Year", placeholder = "Year", label_visibility="collapsed", max_chars=4, disabled = txt_disabled)
        with month:
            file_month = st.text_input("Month", placeholder = "Month", label_visibility="collapsed", max_chars=2, disabled = txt_disabled)
        with day:
            file_day = st.text_input("Day", placeholder = "Day", label_visibility="collapsed", max_chars=2, disabled = txt_disabled)

        if file_year and file_month and file_day:
            try:
                int(file_day)
                int(file_month)
                int(file_year)
        ...
        file_button_disabled = not (to_sell and new_cust_first_name and new_cust_last_name and file_year and file_month and file_day and not txt_disabled)

        if st.button("File Sale for New Customer", disabled = file_button_disabled):
            st.success("Customer created")
            # logic for creating new customer

            st.success("sale filed")
        ...
        year, month, day = st.columns(3)
        with year:
            file_year = st.text_input("Year", placeholder="Year", label_visibility="collapsed", max_chars=4,
                                      disabled=txt_disabled)
        with month:
            file_month = st.text_input("Month", placeholder="Month", label_visibility="collapsed", max_chars=2,
                                       disabled=txt_disabled)
        with day:
            file_day = st.text_input("Day", placeholder="Day", label_visibility="collapsed", max_chars=2,
                                     disabled=txt_disabled)
        ...
        if cust_id:
            try:
                ...
                file_button_disabled = not (to_sell and cust_id and file_year and file_month and file_day and not txt_disabled)
            except ValueError:
                st.error("Must be a number.")
                file_button_disabled = True

        if st.button(f"File Sale for Customer ID: {cust_id}", disabled = file_button_disabled):
            st.success("Sale filed ")

with delete_sale:
    st.subheader("Delete a Sale", divider = "gray")

    to_delete = st.text_input("Enter ID of Sale to Delete")
    ...
    if to_delete:
        try:
            ...
            if to_delete not in temp_sales_df["Sale ID"]:
    ...
    if st.button(f"Delete Sale ID: {to_delete}", disabled = delete_button_disabled):
